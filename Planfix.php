...
namespace common\components\planfix\src;

/**
 * https://planfix.ru/ API-client
 *
 * Class Planfix
 */
class Planfix
{
    ...
    public function auth()
    {
        if (!($this->getUserLogin() && $this->getUserPassword())) {
            throw new \ErrorException('User credentials are not set');
        }
        ...
        if (!$response['success']) {
            throw new \Exception('Unable to authenticate: ' . $response['error_message']);
        }
